...
## 2025-02-18 - Missing OIDC Audience Verification
**Vulnerability:** The application was skipping verification of the OIDC token (`Authorization: Bearer <jwt>`) audience claim for Cloud Tasks webhooks, meaning any validly signed Google token could trigger the internal dispatcher endpoint regardless of its intended recipient.
**Learning:** Placeholders intended for "future" implementations often act as security bypasses if deployed to production without additional constraints. When delegating authentication to external identity providers, validating the audience (`aud` claim) is critical to prevent cross-service token replay attacks.
**Prevention:** Always implement full verification of JWT/OIDC tokens, including audience matching, using established libraries like `google.golang.org/api/idtoken`. Do not leave security-critical verification steps as "no-op" placeholders in production-bound code.
