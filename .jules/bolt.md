...
## 2025-02-18 - Optimize streaming logs rendering to prevent O(N²) cascade
**Learning:** Found an O(N²) rendering bottleneck in `frontend/src/pages/BuildLogs.jsx` where a continuously appending WebSocket log array caused the entire DOM list of logs to be re-rendered and strings repeatedly processed (`toLowerCase()`) on every new message.
**Action:** In React applications, when rendering append-only stream items (such as streaming WebSocket logs), extract the individual items into separate components wrapped in `React.memo()` to prevent O(N²) bottlenecks caused by re-evaluating expensive operations (like string parsing) across the entire list on every state update.
