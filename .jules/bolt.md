...
## 2026-08-14 - Extract streaming log items into memoized components to prevent O(N^2) re-renders
**Learning:** When rendering streaming updates (like WebSocket build logs) in React, if the log lines are rendered inline using `.map()` within the main component, every new line added triggers a re-render and re-evaluation (like string formatting or parsing) of *all previous lines*. This creates an O(N²) bottleneck causing massive UI lag.
**Action:** Always extract items of append-only lists (like streaming logs) into separate components wrapped in `React.memo()`. This ensures that only new elements render and older elements skip re-rendering entirely.
