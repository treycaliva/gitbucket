...
## 2025-02-18 - Extract complex list items into memoized components to avoid cascading O(N^2) re-renders
**Learning:** Found an O(N^2) rendering bottleneck in `frontend/src/pages/BuildLogs.jsx` where a live-streaming log array mapped each incoming line directly in the `BuildLogs` component render loop. As new logs arrived, all prior logs were re-rendered, triggering redundant `toLowerCase()`, `includes()`, and `startsWith()` string parsing for thousands of elements.
**Action:** Extract append-only list items into separate components wrapped in `React.memo` so string parsing and rendering styles are evaluated exactly once per log line. Always avoid inline `.map` rendering with complex parsing in fast-updating lists.
