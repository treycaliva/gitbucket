...
## 2025-02-18 - Optimize append-only stream rendering with memoized components
**Learning:** Found O(N²) bottlenecks in `frontend/src/pages/BuildLogs.jsx` when streaming large lists of logs over WebSockets. Because the stream is append-only, mapping the entire array on every single incoming message and re-evaluating string operations (like `line.toLowerCase().includes('error')`) blocks the main thread severely as the log grows.
**Action:** Extract individual append-only list items into separate components wrapped in `React.memo()`. This ensures that previously parsed and rendered elements are not re-evaluated when new elements are pushed to the array.
