...
## 2025-03-08 - Optimize diff file tree stats calculation
**Learning:** Found an O(N * D) algorithmic bottleneck in `frontend/src/pages/Repository.jsx` `calculateTreeStats` where tree node additions and deletions were being calculated in a separate recursive pass across the entire node tree. This recursive reduction recalculates stats from the bottom up on all nodes.
**Action:** Pre-aggregate sums incrementally during construction instead of a separate O(N*D) recursive post-processing pass. When constructing deep hierarchies (e.g., file trees) where parent nodes depend on child node metrics, always pre-aggregate these sums incrementally during the initial creation phase.
