...
## 2026-07-19 - Pre-aggregate array metrics in a single pass to eliminate O(N*D) bottlenecks
**Learning:** Found an `O(N * D)` algorithmic bottleneck in `frontend/src/pages/Repository.jsx` where tree statistics (like `additions` and `deletions`) for PRs were being calculated with a recursive `calculateTreeStats` function utilizing `reduce()`. For PRs with large file hierarchies, re-traversing the already constructed deep arrays using recursive reductions triggered severe performance delays.
**Action:** When constructing deep hierarchies (like file trees) where parent nodes depend on child node metrics, pre-aggregate these sums incrementally directly during the initial creation phase instead of requiring a secondary recursive evaluation pass. This maintains an `O(N)` algorithmic complexity.
