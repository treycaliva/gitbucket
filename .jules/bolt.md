...
## 2025-02-18 - Memoize append-only streaming logs in React
**Learning:** Rendering continuous, high-volume append-only arrays (like WebSocket logs) inside an inline `.map()` causes O(N^2) render performance degradation because React evaluates string parsing logic on every historic item every time a new item arrives.
**Action:** Extract streaming list items into a discrete component and wrap it in `React.memo()`. This ensures older elements skip re-rendering when the parent array is appended to. Also, never add new testing frameworks or alter package.json without explicit instruction, even when attempting to add unit tests for verification.
