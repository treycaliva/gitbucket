...
## 2025-02-18 - Optimize append-only stream items with React.memo()
**Learning:** Found O(N²) string parsing bottleneck in `frontend/src/pages/BuildLogs.jsx` where the entire log array re-evaluated its string properties (e.g. `.toLowerCase()`, `.includes()`) every time a new log line was appended from the WebSocket stream.
