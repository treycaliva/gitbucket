...
## 2025-02-18 - Optimize rendering of append-only stream items in React
**Learning:** Found an O(N²) rendering bottleneck in `frontend/src/pages/BuildLogs.jsx` where appending a single new WebSocket log line caused React to re-evaluate and re-render every previous log line in the stream. Inline `.map()` rendering for list items that have expensive parsing (like `.toLowerCase()`) kills performance.
**Action:** In React applications, when rendering append-only stream items (such as streaming WebSocket logs), extract the individual items into separate components wrapped in `React.memo()` to prevent O(N²) bottlenecks caused by re-evaluating expensive operations across the entire list on every state update.
